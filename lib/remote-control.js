(function() {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.clear_metrics = async function(me) {
    return (await me.page._client.send('Emulation.clearDeviceMetricsOverride'));
  };

  //-----------------------------------------------------------------------------------------------------------
  this.emulate_media = async function(me) {
    var ref;
    await me.page.emulateMediaType((ref = me.emulate_media) != null ? ref : 'print');
  ...
  this.new_remote_control = async function(settings) {
    var R, browser_settings, page_loaded, ref, ref1;
    R = {};
    settings = merge({}, require('./settings'), settings);
    settings.puppeteer.headless = (ref = !settings.gui) != null ? ref : true;
    browser_settings = settings[(ref1 = settings.use_profile) != null ? ref1 : 'puppeteer'];
