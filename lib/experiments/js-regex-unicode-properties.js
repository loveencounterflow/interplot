(function() {
  //###########################################################################################################
  var CND, alert, badge, cid_matches_pattern, debug, declare, echo, format_integer, help, i, info, isa, len, log, pattern, patterns, ranges_from_pattern, rpr, show_ranges, size_of, type_of, types, urge, validate, warn, whisper;
  ...
  show_ranges = function(ranges) {
    ...
    for (j = 0, len1 = ranges.length; j < len1; j++) {
      ...
      glyphs = [];
      for (cid = k = ref = first_cid, ref1 = Math.min(first_cid + 32, last_cid); (ref <= ref1 ? k <= ref1 : k >= ref1); cid = ref <= ref1 ? ++k : --k) {
