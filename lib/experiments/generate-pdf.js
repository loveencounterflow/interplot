(function() {
  ...
  demo_2 = async function() {
    var browser, chr, delta, i, j, len, len1, media, page, path, pdf, rectangle, ref, target_selector, text, url;
...
// await page.keyboard.down 'Shift'
    for (j = 0, len1 = text.length; j < len1; j++) {
      chr = text[j];
      rectangle = (await page.evaluate(function() {
        return OPS.rectangle_from_selection();
      }));
