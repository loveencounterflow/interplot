(function() {
  'use strict';
  var $, CND, FS, LINEMAKER, OPS_slug_from_slabs, PATH, PD, PUPPETEER, _format, _raw_font_stats_from_selector, after, alert, assign, async, badge, cast, debug, demo_2, demo_insert_slabs, echo, echo_browser_console, format_as_percentage, format_float, format_integer, get_page, get_ppt_doc_object, help, info, isa, join_path, jr, mark_blocks_with_fallback_glyphs, page_html_path, rpr, settings, show_global_font_stats, sleep, take_screenshot, type_of, types, urge, validate, warn, whisper,
    indexOf = [].indexOf;
  ...
  mark_blocks_with_fallback_glyphs = async function(page) {
    var block_selectors, doc, font, i, j, len, len1, raw_font_stats, ref, selector;
    block_selectors = (await page.evaluate(function() {
  ...
  show_global_font_stats = async function(page) {
    ...
    var R, d, doc, fallbacks, font, font_name, font_name_txt, glyph_count, glyph_count_txt, glyph_total, i, len, percentage_txt, raw_font_stats;
    doc = (await get_ppt_doc_object(page));
  ...
  OPS_slug_from_slabs = async function(page, ...P) {
    return (await page.evaluate((function(...P) {
      return OPS.slug_from_slabs(...P);
    }), ...P));
  };

  //-----------------------------------------------------------------------------------------------------------
  demo_insert_slabs = async function(page) {
    var XXX_settings, html, slabs, text;
