(function() {
  ...
  provide_ops = function() {
    ...
    //-----------------------------------------------------------------------------------------------------------
    this._get_partial_slug = function(slabs_dtm, min_slab_idx, max_slab_idx) {
      /* TAINT also return text usable on next invocation so that part will not have to be computed again */
      var i, is_first_slab, is_last_slab, ref, ref1, rhs, slab_idx, spc_count, text, txt;
      text = '';
      spc_count = 0;
    ...
    */
    ...
    return this.slugs_with_metrics_from_slabs = async function(slabs_dtm, settings) {
      ...
      while (true) {
        ...
        partial_slug = this._get_partial_slug(slabs_dtm, min_slab_idx, max_slab_idx);
