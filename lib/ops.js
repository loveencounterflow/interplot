(function() {
  ...
  provide_ops = function() {
    ...
    //-----------------------------------------------------------------------------------------------------------
    this._context_from_linenr = function(line_nr) {
      var R;
      R = {};
    ...
    //-----------------------------------------------------------------------------------------------------------
    this._metrics_from_partial_slug = function(ctx, partial_slug) {
      /* NOTE flag must always have a nominal height of 1mm */
      /* NOTE precision only applied for readability */
      var epsilon, free_width_mm, free_width_px, free_width_rel, lflag_rect, line_too_long, precision, rflag_rect, spc_width_mm, spc_width_px, text_width_mm, text_width_px, text_width_rel;
    ...
    return this.slug_from_slabs = function(slab_dtm, settings) {
      ...
      var ctx, defaults, last_slab_idx, line_nr, max_slab_idx, min_slab_idx, partial_slug, slabs, slug_metrics;
      defaults = {
        min_slab_idx: 0
      };
      ...
      while (true) {
        ...
        line_nr++;
        ctx = this._context_from_linenr(line_nr);
