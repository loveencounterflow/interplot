  ...
  Micro_dom = class Micro_dom { // extends Multimix
    ...
    insert(position, target, dlm) {
      switch (position) {
        case 'before':
        case 'beforebegin':
          return this.insert_before(target, dlm);
        case 'as_first':
        case 'afterbegin':
          return this.insert_as_first(target, dlm);
        case 'as_last':
        case 'beforeend':
          return this.insert_as_last(target, dlm);
        case 'after':
        case 'afterend':
          return this.insert_after(target, dlm);
      }
    ...
    insert_before(target, dlm) {
      V.delement(target);
      return target.insertAdjacentElement('beforebegin', dlm);
    }

    insert_as_first(target, dlm) {
      V.delement(target);
      return target.insertAdjacentElement('afterbegin', dlm);
    }

    insert_as_last(target, dlm) {
      V.delement(target);
      return target.insertAdjacentElement('beforeend', dlm);
    }

    insert_after(target, dlm) {
      V.delement(target);
      return target.insertAdjacentElement('afterend', dlm);
    }

    //-----------------------------------------------------------------------------------------------------------
    before(target, ...dlms) {
      V.delement(target);
      return target.before(...dlms);
    }

    prepend(target, ...dlms) {
      V.delement(target);
      return target.prepend(...dlms);
    }

    append(target, ...dlms) {
      V.delement(target);
      return target.append(...dlms);
    }

    after(target, ...dlms) {
      V.delement(target);
      return target.after(...dlms);
    }
